"""
Tests functionality offered by brewblox_history.amqp
"""
...
from brewblox_history import amqp

TESTED = amqp.__name__
...
def mocked_connect(mocker, m_protocol, m_transport):
    ...
    mocker.patch.object(amqp.aioamqp, 'connect', conn_mock_func)
...
async def test_setup(app, client):
    assert amqp.get_listener(app)
    assert amqp.get_publisher(app)
...
async def test_subscribe_callbacks(app, client, mocker, m_channel):
    ...
    sub = amqp.get_listener(app).subscribe(
...
async def test_offline_listener(app, mocker):
    ...
    listener = amqp.EventListener(app)
...
async def test_online_listener(app, client, mocker):
    listener = amqp.get_listener(app)
    amqp.subscribe(app, 'exchange', 'routing')
...
async def test_offline_publisher(app):
    publisher = amqp.EventPublisher(app)
...
async def test_online_publisher(app, client, mocker):
    ...
    await publisher.publish('exchange', 'key', message=dict(key='val'))
    await amqp.publish(app, 'exchange', 'key', message=dict(key='val'))
...
async def test_publisher_exceptions(app, client, m_protocol):
    ...
    with pytest.raises(aioamqp.AmqpClosedConnection):
        await amqp.get_publisher(app).publish('exchange', 'routing', 'message')
